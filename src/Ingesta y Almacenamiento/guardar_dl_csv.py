...
# Ruta base en HDFS donde se guardarán los .csv
ruta_datalake = "/datalake/datos/bronze/csv"

# Ruta temporal dentro del contenedor
ruta_docker_temporal = "/tmp/csv_uploads"

# Ruta local de origen
SOURCE_ROOT = ruta_proyecto / "data" / "datosBigData" / "data" / "csv"
...
FILE_PATTERN = re.compile(r"^\d{4}_\d{2}\.csv$")
...
def is_valid(path: Path) -> bool:
    ...
    Valida rutas locales del tipo:
      SOURCE_ROOT/tile_00_00/2005/2005_07.csv
    """
    try:
...
def main() -> None:
    ...
    files = [p for p in SOURCE_ROOT.rglob("*.csv") if p.is_file()]
