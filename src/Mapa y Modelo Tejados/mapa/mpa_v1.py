...
ctx.drawImage(fullCanvas, startX, startY, CAPTURE_SIZE, CAPTURE_SIZE, 0, 0, CAPTURE_const tl = map.containerPointToLatLng([startX, startY]);
...
fetch("https://nominatim.openstreetmap.org/search?q=" + encodeURIComponent(q) + "&headers: {{ "User-Agent": "SolarMap/1.0" }}
}})
.then(r => r.json())
.then(data => {{
...
<div style="font-weight:700;margin-bottom:6px;">Tejado ${{p.id || ""}}</<div style="font-size:13px;color:#374151;">Área: <strong>${{p.area_m2 <div style="font-size:13px;color:#374151;">Orientación: <strong>${{p.<div style="font-size:13px;color:#374151;">Ángulo: <strong>${{p.orientation_</div>
`);
}}
}}).addTo(map);
}});
