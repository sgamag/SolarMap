# src/modelo_peticion_imagen2.py
...
class ImageRequest:
    ...
    def validate(self) -> None:
        ...
        print(f"[VALIDACION] AOI y fechas correctas: {self.aoi.wkt_4326}, {self.window.start} a {self.window.end}")

    def overlaps_existing(self, existing_dates: list[date]) -> bool:
        """Opcional: revisa si este request solapa con datos ya descargados."""
        for d in existing_dates:
            if self.window.start <= d <= self.window.end:
                return True
        return False
